...
def draw_bounding_box_on_image(image,ymin,xmin,ymax,xmax,color='red',thickness=2,
    ...
    if use_normalized_coordinates: # False
        (left, right, top, bottom) = (xmin * im_width, xmax * im_width, ymin * im_height, ymax * im_height)

    else: # run this line code
...
def data_line_parser(line, dataset):
    ...
    if str1 not in line:
        ...
        str3 = ':'
        str3 = str3.encode()
        img_path, bbox_coordinates_raw = line.split(str3)
        if dataset == 'hollywood':
            src_path = os.path.join(opt.hollywood_dataset_root_path,'Images',img_path.replace('"',''))
        elif dataset == 'brainwash':
            src_path = os.path.join(opt.brainwash_dataset_root_path, img_path.decode().replace('"',''))

        bbox_coordinates_raw = bbox_coordinates_raw.decode().replace('\n', '')
        bbox_coordinates_raw = bbox_coordinates_raw.replace('(','')
        bbox_coordinates_raw = bbox_coordinates_raw.replace(')','')
        bbox_coordinates_raw = bbox_coordinates_raw.replace(';','')
        bbox_coordinates_raw = bbox_coordinates_raw.replace('.0.','.0')
